...
export function ItemPicker({ item: m, leadTimeHours, onClose }: { item: MenuItem; leadTimeHours: number; onClose: () => void }) {
  const { add } = useCart();
  ...
  return (
    ...
    >
      <div className="relative max-h-[92vh] w-full max-w-[420px] overflow-y-auto rounded-[16px] bg-white p-6 pt-5">
        ...
        <div className="mb-3 flex justify-center">
          {m.image_url ? (
            <Image src={m.image_url} alt={m.name} width={130} height={130} className="h-[130px] w-[130px] rounded-[14px] bg-cream2 object-cover" unoptimized />
          ) : (
            <div className="flex h-[90px] w-[90px] items-center justify-center rounded-[14px] bg-rose text-[44px]">{m.emoji}</div>
          )}
        </div>
        ...
        <div className="mb-3 text-center text-[20px] font-bold text-rose-deep">{aed(unit * qty)}</div>
        <button className="btn-p w-full rounded-[10px] py-3" disabled={blocked} onClick={confirm}>
