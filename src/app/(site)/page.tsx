      ...
      <section className="m-fade-up relative -mx-5 -mt-5 mb-6 overflow-hidden">
        {settings.hero_image_url ? (
          ...
          <div className="absolute inset-0 bg-cream2">
            <div
              aria-hidden
              className="m-float absolute -left-8 -top-10 h-40 w-40 rounded-full bg-rose/70 blur-2xl"
            />
            <div
              aria-hidden
              className="m-float absolute -bottom-12 -right-6 h-44 w-44 rounded-full bg-lav/70 blur-2xl [animation-delay:-3s]"
            />
          </div>
        )}
        <div
          className={`relative px-7 pb-8 pt-16 text-center ${settings.hero_image_url ? "text-white" : "text-ink"}`}
        >
          <span
            className={`mb-4 inline-block rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[2px] ${settings.hero_image_url ? "bg-white/15 text-white backdrop-blur" : "bg-rose text-rose-deep"}`}
          >
            Home baked · Dubai · est. 2019
          </span>
          <h1 className="words mx-auto mb-2 max-w-[14ch] text-[38px] leading-[1.08]">
            <span style={{ "--w": 0 } as React.CSSProperties}>Baked</span>{" "}
            <span style={{ "--w": 1 } as React.CSSProperties}>to</span>{" "}
            <span style={{ "--w": 2 } as React.CSSProperties}>
              <em
                className={`font-normal italic ${settings.hero_image_url ? "text-rose" : "text-rose-deep"}`}
              >
                perfection
              </em>
              ,
            </span>{" "}
            <span style={{ "--w": 3 } as React.CSSProperties}>with</span>{" "}
            <span style={{ "--w": 4 } as React.CSSProperties}>soul.</span>
          </h1>
          <p
            className={`mx-auto mb-6 max-w-[380px] text-[14px] leading-[1.7] ${settings.hero_image_url ? "text-white/85" : "text-muted"}`}
          >
            Cookies, cheesecakes, tiramisu and tea cakes made slowly from
            scratch by {first}, and delivered to your door.
          </p>
          <div className="flex flex-wrap justify-center gap-2.5">
            ...
            <Link
              href="/custom-cakes"
              className={`press inline-flex items-center rounded-full border-[1.5px] px-5 py-2.5 text-[13px] transition ${settings.hero_image_url ? "border-white/60 text-white hover:bg-white/10" : "border-rose-deep text-rose-deep hover:bg-rose"}`}
            >
              Custom cakes
            </Link>
          </div>
